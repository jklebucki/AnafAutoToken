            ...
            if (latestToken is not null)
            {
                payload = new CurrentTokenExportFile(
                    ...
                    CurrentToken: new CurrentTokenPayload(
                        ...
                        latestToken.RefreshToken.GetExpirationDate() ?? latestToken.RefreshTokenExpiresAt ?? latestToken.SavedAt.AddDays(365),
    ...
    private async Task<List<TokenListItem>> GetAllTokenLogsAsync(string connectionString)
    {
        ...
        if (hasRefreshTokenExpiresAtColumn)
        {
            return await dbContext.TokenRefreshLogs
                ...
                .Select(log => new TokenListItem(
                    ...
                    log.AccessToken,
                    log.RefreshToken,
                    log.RefreshToken.GetExpirationDate() ?? log.RefreshTokenExpiresAt ?? (log.IsSuccess ? log.CreatedAt.AddDays(365) : null),
        ...
        return await dbContext.TokenRefreshLogs
            ...
            .Select(log => new TokenListItem(
                ...
                log.AccessToken,
                log.RefreshToken,
                log.RefreshToken.GetExpirationDate() ?? (log.IsSuccess ? log.CreatedAt.AddDays(365) : null),
                string.IsNullOrWhiteSpace(log.AccessToken) ? null : log.AccessToken.GetExpirationDate(),
                log.ExpiresAt,
                log.CreatedAt,
                log.IsSuccess,
